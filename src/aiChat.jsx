...
export default function Chat() {
        ...
        if (await confirmAlert({
            ...
            setConversationList(original => {
                ...
                if (newConversation.length === 1) {
                    ...
                    toast(Toast.Style.Success, `Deleted only conversation, "${conversationName}", and restored a new conversation.`)
    ...
    const purgeLocalStorage = async () => {
        if (await confirmAlert({
            title: "Confirm Purge",
            message: "Are you sure you want to completely clear all memory associated with the Google Bard extension? This is an irreversable action, and will result in the loss of all of your conversations, messages, and answers.",
            primaryAction: {
                title: "Continue",
                style: Alert.ActionStyle.Destructive
            },
    ...
    const CreateConversationAction = () => {
        ...
        return (
            ConversationNameForm("Create Conversation", handleSubmitConversation)
        );
    }
    ...
    const RenameConversationAction = () => {
        ...
        return (
            ConversationNameForm("Rename Conversation", handleRenameConversation)
        );
    }
    ...
    const formatDateTime = timestamp => {
        ...
        return inputDate.getDate() === currentDate.getDate() && inputDate.getMonth() === currentDate.getMonth() && inputDate.getFullYear() === currentDate.getFullYear()
            ...
            : `${(inputDate.getMonth() + 1).toString().padStart(2, '0')}/${inputDate.getDate().toString().padStart(2, '0')}/${inputDate.getFullYear()}`;
    };

    // Makes the dropdown to select conversations
    const ConversationDropdown = () => {
        return (
            isLoaded ? <List.Dropdown
        ...
        >
            {(() => {
                ...
                if (!bardWorking) {
                    ...
                    return (
                        getCurrentConversation().questions.map(({ question, response, metadata }, i) => (
                            <List.Item
                                accessories={[
                                    { text: `${formatDateTime(metadata.date)}` },
                                ]}
                                key={question + i}
                                title={question}
                                actions={<BardActionPanel />}
